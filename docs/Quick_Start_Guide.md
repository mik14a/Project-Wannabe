...
このガイドは、小説執筆支援アプリ `Project Wannabe` と、そのバックエンドとして動作する AI エンジン `KoboldCpp` を Windows 環境にセットアップし、連携させて動作確認件、作品作りまでの流れを説明します。
...
*   **オーサーズノート:** 次に生成してほしい内容への具体的な指示（シーン、文体、感情、行動など）を記述します。AIへの直接的な命令として機能します（次期モデル対応予定）。
