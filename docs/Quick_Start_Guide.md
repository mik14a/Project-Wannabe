...
1.  `Project Wannabe` が推奨する `wanabi` モデル（GGUF 形式）をダウンロードします。
    *   **[wanabiシリーズ](https://huggingface.co/collections/kawaimasa/wanabi-683144d1bf44a0b922aa1609)**
2.  ダウンロードした `.gguf` ファイルを、`koboldcpp.exe` と同じフォルダ、または分かりやすい場所に保存します。

<details>
<summary>モデル選択のヒント（VRAM容量別）</summary>

ダウンロードする `wanabi` モデルは、お使いのPCのVRAM（グラフィックボードのメモリ）容量によって、快適に動作するものが異なります。以下は、`koboldcpp.exe` を使用する際の目安です。

**ご自身のVRAM容量を確認し、推奨設定を参考にモデルを選んでみましょう。**

---

##### **VRAM 24GB の場合**

24GBのVRAMがあれば、高品質な設定で `wanabi` モデルを快適に楽しめます。

*   **オプション1: 高品質・長文対応**
    *   **モデル例:** `wanabi_24b_v03_q5km.gguf` 
    *   **KoboldCPP設定目安:**
        *   コンテキスト長: 32768
        *   KVキャッシュ: 8bit 
    *   *補足: 高い生成品質と長い物語の記憶を両立できます。*

*   **オプション2: 最高品質追求**
    *   **モデル例:** `wanabi-24b-Q6_K.gguf`
    *   **KoboldCPP設定目安:**
        *   コンテキスト長: 16384 または、メモリに合わせて調整
        *   KVキャッシュ: 8bit 
    *   *補足: より高い品質を目指せますが、一度に記憶できる文章量は少し短くなる場合があります。*

---

##### **VRAM 16GB の場合**

16GBのVRAMでは、モデルサイズと設定のバランスが重要になります。

*   **オプション1: 24Bモデルに挑戦**
    *   **モデル例:** `wanabi_24b_v03_iq4xs.gguf` や `wanabi_24b_v03_q4km.gguf`
    *   **KoboldCPP設定目安:**
        *   コンテキスト長: ? (未検証)
        *   KVキャッシュ: 8bit (4bitは非推奨)
    *   *補足: 24Bパラメータモデルの品質を体験できますが、記憶できる文章量は短めになります。*

*   **オプション2: 12Bモデルで安定動作・長文対応**
    *   **モデル例:** `wanabi_mini_12b_Q6_K.gguf`
    *   **KoboldCPP設定目安:**
        *   コンテキスト長: 32768 (32K) 以上も狙えます
        *   KVキャッシュ: 8bit
    *   *補足: 安定した動作と長い物語の記憶を重視するならこちら。*

---

##### **VRAM 12GB の場合**

12GBのVRAMでは、12Bパラメータのモデルが主な選択肢となります。

*   **推奨設定:**
    *   **モデル例:** `wanabi_mini_12b_Q4_K_M.gguf` や `wanabi_mini_12b_IQ4_XS.gguf`
`
    *   **KoboldCPP設定目安:**
        *   コンテキスト長: 24576 (24K) 程度で調整
        *   KVキャッシュ: 8bit
    *   *補足: 12GBのVRAMでバランスの取れた設定です。

---

##### **VRAM 8GB 以下 の場合**

正直快適な動作は難しいです。コンテキスト長、品質を大きく下げるか動作速度の大幅な低下のトレードオフを迫られることになります。
全てのレイヤーをVRAMに載せる必要がありません、大きく速度は下がりますが一部をCPU側で処理することも可能です。

*   **モデル例:** `wanabi_mini_12b_IQ4_XS.gguf`
*   **KoboldCPP設定目安:**
    *   コンテキスト長: ？
    *   KVキャッシュ: 4bit

*   *補足: 動作は可能ですが、生成品質や一度に記憶できる文章量には限界があります*
...
1.  ウィンドウ上部のツールバーで **「アイデア出し」** モードを選択します。
    ![アイデア出しモード選択](../img/idea_mode_selected.png)
2.  **生成項目の選択:** アイデア出しモードを選択すると、ツールバーの下（または近く）にドロップダウンリストが表示されます。ここで、AIに生成させたい項目を選びます。例えば、「全体」を選べばタイトルからプロットまで一式、「タイトルのみ」を選べばタイトル案だけを生成します。詳細については付録の「アイデア出しモードのUI要素」を参照してください。
    <!-- TODO: ここに項目選択ドロップダウンのスクリーンショットを挿入 -->
3.  **(任意) 詳細情報の設定:** 右側の「詳細情報」タブを開き、生成したいアイデアの方向性を AI に伝えるために、いくつかの項目を入力します。タブ内の各セクションはクリックで開閉できます。
    ...
    *   **【注意事項】**
        ...
        *   **レーティング、オーサーズノート、セリフ量** の項目は、アイデア出し時のプロンプトには基本的に影響しません（これらは主に小説生成モードで利用されます）。
4.  メニューバーの `生成` > `単発生成 (Ctrl+G)` をクリックします。
5.  左下の出力エリアに、AI が生成したアイデア（例: `# タイトル: ...`, `# キーワード: ...`, `# あらすじ: ...` のような形式）が表示されるのを待ちます。
...
自由入力欄の項目は特定のフォーマットはありませんが、またアイデア出し時に出てくるフォーマットは確実にサポートしています。
例えばplot欄
```
起:
承:
...
結:
```
などです。アイデアが固まっている人も一度試してみると勝手が分かると思います。（面倒くさいかたは箇条書き等の構造化をせず、平文で書くのが安全です。）
...
**D. アイデア出しモードのUI要素 (ツールバー下部)**

アイデア出しモードを選択した際に、ツールバーの下（または近く）に表示される追加のUI要素です。

*   **生成項目 (ドロップダウンリスト):**
    *   **目的:** AIに生成させたい小説の構成要素を選択します。
    *   **選択肢の例:** 「全体」（タイトル、キーワード、ジャンル、あらすじ、設定、プロットを一式生成）、「タイトルのみ」、「キーワードのみ」、「ジャンルのみ」、「あらすじのみ」、「設定のみ」、「プロットのみ」など。
    *   **動作:** 選択した項目に応じて、AIへの指示（内部的なStop Sequenceなど）が調整されます。
*   **高速な手法（実験的） (チェックボックス):**
    *   **目的:** より迅速な応答を目指す実験的な生成手法を有効にするかを選択します。
    *   **有効条件:** このチェックボックスは、「生成項目」で**「全体」以外**の特定の項目（例: 「タイトルのみ」）を選択した場合にのみ有効になります。「全体」を選択した場合は無効（グレーアウトなど）になります。
    *   **動作（チェック時）:** 速度を優先した手法で生成を試みます。常にストリーミング表示に対応します。ただし、実験的な機能のため、結果が不安定になる可能性があります。また、将来実装予定の「補間タスク」とは互換性がありません。
    *   **動作（非チェック時）:** 安定性を重視した手法で生成されます。
    *   **推奨:** 通常はチェックを入れずに使用することをお勧めします。
*   **【ストリーミング表示に関する注意】**
    *   「高速な手法」にチェックを入れた場合（※全体生成時を除く）は、ストリーミング表示されます。
    *   「高速な手法」にチェックを入れず、「全体」を選択した場合も、ストリーミング表示されます。
    *   「高速な手法」にチェックを入れず、かつ特定の項目（例: 「タイトルのみ」）を選択した場合、出力欄へのストリーミング表示が無効になります。これは後処理（内部的なフィルタリング）のためであり、生成自体は正常に行われています。結果は一度にまとめて表示されます。
